'''
    File name      : rdf_dr_cu.py
    Author         : Jinwook Jung
    Created on     : Thu 30 Oct 2020 09:46:48 PM EDT
    Last modified  : 2020-10-31 01:05:11
    Description    : 
'''
...
def run(config, stage_dir, prev_out_dir, user_parms, write_run_scripts=False):
    ...
    dr_cu = DrCuRunner(config, stage_dir, prev_out_dir, user_parms)
    dr_cu.write_run_scripts()
...
class DrCuRunner(Stage):
    ...
    def write_run_scripts(self):
        ...
        cmd += "${RDF_TOOL_BIN_PATH}/detail_route/dr_cu/dr_cu"
        cmd += " dr_cu.parm"
        cmds.append(cmd)
    ...
    def _write_parm_file(self):
        with open("{}/dr_cu.parm".format(self.stage_dir), 'w') as f:
            f.write("lef:{}\n".format(self.lef_mod))
            f.write("def:{}\n".format(self.in_def))
